...
impl<'ra> Module<'ra> {
    ...
    fn for_each_child_mut<'tcx, R: AsMut<Resolver<'ra, 'tcx>>>(
        ...
        // The callback may perform name resolution and therefore borrow this
        // module's resolutions again. Snapshot the children so no `RefCell`
        // borrow is held while the callback runs.
        let children = resolver
            ...
            .filter_map(|(key, name_resolution)| {
                ...
                name_resolution
                    .best_decl()
                    .map(|decl| (key.ident, name_resolution.orig_ident_span, key.ns, decl))
            })
            .collect::<Vec<_>>();
        for (ident, orig_ident_span, ns, decl) in children {
            f(resolver, ident, orig_ident_span, ns, decl);
        }
    }
