...
/// Fast union-find implementation which uses an [`Idx`] as a key type.
///
...
pub struct UnionFind<K: Idx> {
    table: Vec<UnionFindEntry<K>>,
}

impl<K: Idx> UnionFind<K> {
    pub fn new() -> Self {
        Self { table: vec![] }
    }
    ...
    pub fn reset(&mut self, num_keys: usize) {
        self.table.clear();
        self.table.extend((0..num_keys).map(|i| UnionFindEntry { parent: K::new(i), rank: 0 }));
    }
    ...
    pub fn try_union(&mut self, a: K, b: K, unify: impl FnOnce(K, K) -> bool) {
        ...
        let (leader, follower) =
            if self.table[a.index()].rank >= self.table[b.index()].rank { (a, b) } else { (b, a) };
